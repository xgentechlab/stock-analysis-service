...
Uses Alpha Vantage as primary source with NSE Direct API as fallback
"""
import pandas as pd
import requests
...
class StocksService:
    ...
    def fetch_ohlcv_data(self, symbol: str, days: int = 60, max_retries: int = 3) -> Optional[pd.DataFrame]:
        """
        Fetch OHLCV data for a symbol using Alpha Vantage as primary source with NSE and yfinance fallback
        ...
        try:
            ...
            logger.info(f"Fetching OHLCV data for {clean_symbol} using Alpha Vantage")
            
            # Try Alpha Vantage first
            try:
                av_data = self.alpha_vantage_service.fetch_ohlcv_data(clean_symbol, days)
                if av_data is not None and not av_data.empty:
                    logger.info(f"Successfully fetched data from Alpha Vantage for {clean_symbol}")
                    return av_data
                else:
                    logger.warning(f"Alpha Vantage returned no data for {clean_symbol}, trying NSE fallback")
            except Exception as e:
                logger.warning(f"Alpha Vantage failed for {clean_symbol}: {e}, trying NSE fallback")
            
            # Try NSE Direct API as second option
            try:
                nse_data = self.nse_service.fetch_ohlcv_data(clean_symbol, days)
                if nse_data is not None and not nse_data.empty:
                    ...
                    logger.warning(f"NSE Direct API returned no data for {clean_symbol}, trying yfinance fallback")
            except Exception as e:
                logger.warning(f"NSE Direct API failed for {clean_symbol}: {e}, trying yfinance fallback")
            
            # Fallback to yfinance if NSE fails
            logger.info(f"Using yfinance fallback for {clean_symbol}")
            ...
            # Light rate limiting - yfinance 0.2.54+ has fixed the excessive request bug
            current_time = time.time()
            time_since_last_request = current_time - self.last_request_time
            if time_since_last_request < 1.0:  # Reduced from 2.0s to 1.0s
                delay = 1.0 - time_since_last_request + random.uniform(0.2, 0.5)
                ...
                try:
                    ...
                    if not hist.empty:
                        ...
                        if attempt < max_retries - 1:
                            retry_delay = (attempt + 1) * 1 + random.uniform(0.5, 1.5)  # Reduced delays
                    ...
                    if attempt < max_retries - 1:
                        retry_delay = (attempt + 1) * 1.5 + random.uniform(1, 2)  # Reduced delays
                        logger.info(f"Retrying in {retry_delay:.2f}s...")
                        time.sleep(retry_delay)
                    else:
                        raise fetch_error
    ...
    def get_current_price(self, symbol: str) -> Optional[float]:
        """Get current/latest price for a symbol using Alpha Vantage with NSE and yfinance fallback"""
        try:
            # Clean symbol name
            clean_symbol = symbol.replace('.NS', '')
            ...
            try:
                ...
                if nse_price is not None:
                    ...
                    logger.warning(f"NSE Direct API returned no price for {clean_symbol}, trying yfinance fallback")
            except Exception as e:
                logger.warning(f"NSE Direct API failed for {clean_symbol}: {e}, trying yfinance fallback")
            
            # Fallback to yfinance
            logger.info(f"Using yfinance fallback for current price of {clean_symbol}")
